...
namespace WidgetkitEx\SlideshowEx{
...
class WKDiskItem
{
    ...
    private static function printDiskItem($value)
    {
        ...
        if ($value->contents) {
            ...
            $list .= self::printStructureInt($value->contents, true);
            $list .= '</li></ul>';
        } else {
            //File
    ...
    private static function printStructureInt($array, $nested = false)
    {
        ...
        if (is_array($array)) {
            foreach ($array as $value) {
                $list .= self::printDiskItem($value);
            }
        } else {
            $list .= self::printDiskItem($array);
        }
        if (!$nested) {
            $list .= '</ul>';
        }
    ...
    {
        return self::printStructureInt($this);
    }
    ...
    public function toArrayItem()
    {
        ...
        return ['name'=>$this->relativename, 'size'=>$this->size, 'hash'=>$this->hash];
    }
...
class WidgetkitExPlugin
{
    ...
    public function __construct($appWK, $id = 0)
    {
        ...
        } elseif (version_compare(self::stablePHPVersion, $php_version) > 0) {
        ...
        } elseif (preg_match_all('@.*\/wp-content\/plugins\/widgetkit\/plugins\/'.($this->isWidget ? 'widgets' : 'content').'\/.+@', $ipath)) {
            array_push($this->debug_info, 'Installation path is correct');
            $this->pathCorrect = true;
        } else {
    ...
    private function extractWKUserName($widgetkit_user, $firstName = true)
    {
        ...
        if ($firstName) {
            return (count($split_name) > 0) ? $split_name[0] : $name;
        }
        @array_shift($split_name);

        return (count($split_name) > 0) ? implode(' ', $split_name) : '';
    }
...
EOT;
    ...
    public function generateUpdaterJS($appWK, $settings = [])
    {
        ...
        if ((!isset($settings['infotimeout'])) || (!is_int($settings['infotimeout']))) {
...
EOT;
    ...
    private function generateUpdateInfoDialog($appWK)
    {
        $js = <<< EOT
<div class="uk-modal-header">
	<h1>{$appWK['translator']->trans('%name% plugin update details', ['%name%' => $this->plugin_info['name']])}</h1>
</div>
<div class="uk-overflow-container">
	<div class="uk-grid">
